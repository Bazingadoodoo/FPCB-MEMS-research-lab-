...
interval = 0.1;             % precision of delay time (ie. a precision of 0.5Hz means delay times of increments of 0.5ms used)
...
graph_width = 500;
data = zeros(1,graph_width);

%delete(instrfind);
%serial_object = serial('COM3','BaudRate',38400,'DataBits',16);
%serial_object = serial('COM3','BaudRate',38400,'ReadAsyncMode','continuous');
serial_object = serialport('COM3', 38400);
configureCallback(serial_object,"terminator",@callbackSerial)
fopen(serial_object);
% fclose(serial_object);
% clear serial_object

function callbackSerial(obj,~)
    global data graph_width;
    v = fscanf(obj);
    value = str2double(v);
    %disp(value);
